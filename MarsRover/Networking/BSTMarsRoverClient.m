...
@implementation BSTMarsRoverClient
...
- (void)fetchMissionManifestForRoverNamed:(NSString *)name completion:(void(^)(BSTMarsRover *rover, NSError *error))completion;
{
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        completion([[BSTMarsRover alloc] initWithDictionary:manifest], nil);
    }] resume];
}
    
- (void)fetchPhotosFromRover:(BSTMarsRover *)rover onSol:(NSInteger)sol completion:(void (^)(NSArray *, NSError *))completion
{
    if (!rover) {
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        for (NSDictionary *dict in photoDictionaries) {
            BSTMarsPhoto *photo = [[BSTMarsPhoto alloc] initWithDictionary:dict];
            if (!photo) { continue; }
